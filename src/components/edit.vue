<template>
  <div>
    <div class="window full shadow" :style="shadowstyle"></div>
    <div class="window win container-fluid" ref="editbox" :style="style">
      <div class="winbar d-flex" v-hammer:pan="pan" v-hammer:panend="panend">
        <div class="barbtns">
          <div
            :style="'float: left;width: 10px;height: 10px;margin-top: 9px;margin-left: 6px;background-color:'+tikstate"
          ></div>
          <span class="times">{{this.el.begintime}} -- {{this.el.endtime}}</span>
...
export default {
  ...
  methods: {
    ...
    post: function () {
      if (this.type == "new") {
        ...
        req
          ...
          .then((res) => {
            if (res.data != "mis") {
              this.$bus.emit("change" + this.el.id, {
                title: this.newtitle,
                cmt: this.newcmt,
              });
            }
    ...
    setpoz: function () {
      ...
      this.p.Y = poz.keepInWin(
        this.mousepoz.y,
        this.$refs.editbox.offsetHeight,
        div.clientHeight
      );
